...
export default function Home() {
  ...
  const [violationsData, setViolationsData] =
  ...
  useEffect(() => {
    if (violationData) {
      const violations: Violation[] = violationData.personalViolationsData.map(
        (v: { violationTicketNumber: any; violationAmount: any; vehiclePlateCode: any; vehiclePlateNumber: any; violationDate: any; violationPlace: any; violationType: any; }) => ({
  ...
  const handlePaymentSubmit = () => {
    ...
    if (violationsData) {
      const updatedViolations = violationsData.data.violations.filter(
        (v) => !selectedViolations.includes(v.id)
      );
  ...
  return (
    ...
    >
      ...
      <header className="bg-[#eceae4] py-2 px-4 flex justify-between items-center border-b">
        <div className="flex items-center gap-2">
          ...
          <div className="text-right">
            <h1 className="text-[#0a2463] font-bold text-lg leading-tight">
              دولة الكويت
            </h1>
            <h2 className="text-[#0a2463] text-sm leading-tight">
              وزارة الداخلية
            </h2>
          </div>
        </div>
      ...
      <div className="bg-[#0a2463] text-white py-1 px-4 border-b flex">
        <div className="flex border rounded overflow-hidden">
          <Button
            variant="ghost"
            size="sm"
            className="h-8 px-3 border-l rounded-none"
          >
          ...
          <Button
            variant="ghost"
            size="sm"
            className="h-8 w-8 rounded-none flex justify-center items-center"
          >
        ...
        <Button
          variant="ghost"
          size="icon"
          className="text-white mr-auto bg-[#0a2463] hover:bg-[#0a2463]/90"
        >
      ...
      <div className="bg-[#eceae4] py-3 px-4 flex justify-center">
        <div className="text-center">
          <Image
            ...
            className="mx-auto"
          />
          <h3 className="text-[#0a2463] font-bold mt-1 text-sm">
      ...
      <form
        ...
        className="bg-[#eceae4] p-4 border-t border-b"
      >
        <h3 className="text-[#0a2463] font-bold mb-4 text-center text-sm">
          الإدارة العامة للمرور
        </h3>
            ...
            <div className="flex justify-between">
              <div className="font-bold">
                ...
                <span className="text-[#0a2463]">
                  {violationsData.data.totalViolations}
                </span>
              </div>
              <div className="font-bold">
                المبلغ الإجمالي:{" "}
                <span className="text-[#0a2463]">
                  {violationsData.data.totalAmount} د.ك
                </span>
              </div>
            </div>
          </div>
          ...
          {violationsData.data.violations.length > 0 ? (
            <>
              {violationsData.data.violations.map((violation) => (
                <div
                  key={violation.id}
                  className="border-t-4 border-[#008000] bg-white mb-2 shadow-sm mx-4"
                >
                  <div className="flex justify-between items-center p-3 border-b">
                    <div className="flex items-center">
                      ...
                      >
                        رقم:{" "}
                        <span className="text-[#0a2463]">{violation.id}</span>
                      </Label>
                    </div>
                    <Button
                      variant="ghost"
      ...
      <div className="bg-white p-4">
        <div className="flex flex-col items-center mb-4">
          <h3 className="text-[#0a2463] font-bold text-sm">
            الإستعلام عن رقم مرجع الداخلية
          </h3>
        ...
        <Button className="w-full bg-[#e6e6e6] hover:bg-[#d9d9d9] text-[#333] font-medium py-2 mb-2">
          للكويتيين
        </Button>

        <Button className="w-full bg-[#e6e6e6] hover:bg-[#d9d9d9] text-[#333] font-medium py-2">
          للمقيمين
        </Button>
      </div>
      ...
      <footer className="bg-white py-3 px-4 text-center text-xs">
        ...
        <p className="text-[#555] text-[10px]">
          © جميع الحقوق محفوظة - وزارة الداخلية الكويت - 2023
        </p>
      </footer>
    </div>
  );
